...
-- checks to see highest buyer
 select first_name, last_name, sum(amount) as am_spent from customers join orders on customers.id = orders.customer_id  group by first_name, last_name order by am_spent desc;
...
-- checks for null values & displays zero;
select first_name,last_name, ifnull(sum(amount), 0) as amount from customers left join orders on customers.id = orders.customer_id group by first_name,last_name order by amount desc;
